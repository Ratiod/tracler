...
async function initDatabase() {
  ...
  // Add columns to playbooks if they don't exist (migration)
  try { await db.execute(`ALTER TABLE playbooks ADD COLUMN name TEXT`); } catch {}
  try { await db.execute(`ALTER TABLE playbooks ADD COLUMN map TEXT`); } catch {}
  try { await db.execute(`ALTER TABLE playbooks ADD COLUMN desc TEXT`); } catch {}
  try { await db.execute(`ALTER TABLE playbooks ADD COLUMN pdf_url TEXT`); } catch {}
  try { await db.execute(`ALTER TABLE playbooks ADD COLUMN file_name TEXT`); } catch {}
  // Add maps/opp/side/date to gameplans if they don't exist
  try { await db.execute(`ALTER TABLE gameplans ADD COLUMN maps TEXT`); } catch {}
  try { await db.execute(`ALTER TABLE gameplans ADD COLUMN opp TEXT`); } catch {}
  try { await db.execute(`ALTER TABLE gameplans ADD COLUMN side TEXT`); } catch {}
  try { await db.execute(`ALTER TABLE gameplans ADD COLUMN date TEXT`); } catch {}
  ...
  await db.execute(`CREATE TABLE IF NOT EXISTS vods (id TEXT PRIMARY KEY, title TEXT NOT NULL, folder TEXT NOT NULL DEFAULT 'Scrims', url TEXT NOT NULL DEFAULT '', ts TEXT NOT NULL DEFAULT '[]', created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
...
app.post('/api/playbooks', async (req, res) => { try { const { id, name, map, desc, pdfUrl, fileName } = req.body; await db.execute({ sql: 'INSERT INTO playbooks (id,title,name,map,desc,pdf_url,file_name) VALUES (?,?,?,?,?,?,?)', args: [id, name||'Playbook', name||'', map||'', desc||'', pdfUrl||'', fileName||''] }); res.json({ success: true }); } catch (e) { res.status(500).json({ error: e.message }); } });
...
app.post('/api/compositions', async (req, res) => { try { const { id, map, roles } = req.body; await db.execute({ sql: 'INSERT INTO compositions (id,map,roles) VALUES (?,?,?)', args: [id, map, typeof roles === 'string' ? roles : JSON.stringify(roles)] }); res.json({ success: true }); } catch (e) { res.status(500).json({ error: e.message }); } });
...
// VOD Reviews
app.get('/api/vods', async (req, res) => { try { res.json((await db.execute('SELECT * FROM vods ORDER BY created_at DESC')).rows); } catch (e) { res.status(500).json({ error: e.message }); } });
app.post('/api/vods', async (req, res) => { try { const { id, title, folder, url } = req.body; await db.execute({ sql: 'INSERT INTO vods (id,title,folder,url) VALUES (?,?,?,?)', args: [id, title, folder||'Scrims', url||''] }); res.json({ success: true }); } catch (e) { res.status(500).json({ error: e.message }); } });
app.put('/api/vods/:id', async (req, res) => { try { const { title, folder, url, ts } = req.body; await db.execute({ sql: 'UPDATE vods SET title=?,folder=?,url=?,ts=? WHERE id=?', args: [title, folder||'Scrims', url||'', JSON.stringify(ts||[]), req.params.id] }); res.json({ success: true }); } catch (e) { res.status(500).json({ error: e.message }); } });
app.delete('/api/vods/:id', async (req, res) => { try { await db.execute({ sql: 'DELETE FROM vods WHERE id = ?', args: [req.params.id] }); res.json({ success: true }); } catch (e) { res.status(500).json({ error: e.message }); } });
