...
function VodReview() {
  ...
  const [tsForm, setTsForm]       = useState({ time:"0:00", cat:"Mistake", note:"" });
  ...
  // Parse ts field which comes as JSON string from DB
  const parseVod = v => ({ ...v, ts: typeof v.ts === "string" ? JSON.parse(v.ts || "[]") : (v.ts || []) });

  // Load VODs from API on mount
  useEffect(()=>{
    setLoading(true);
    api.get("/api/vods").then(d=>{
      if(Array.isArray(d)) setVods(d.map(parseVod));
  ...
  const addVod = () => {
    ...
    const id = `vod-${Date.now()}`;
    const v = { id, title:newForm.title, folder:newForm.folder, url:newForm.url, ts:[] };
    api.post("/api/vods", { id, title:newForm.title, folder:newForm.folder, url:newForm.url })
      .then(()=>{
        setVods(p=>[v,...p]);
        setSel(v);
      })
  ...
  const addTimestamp = () => {
    if(!tsForm.note.trim()||!sel) return;
    const newTs = { time:tsForm.time, cat:tsForm.cat, note:tsForm.note, replies:[] };
    const updatedTs = [...(sel.ts||[]), newTs];
    api.put(`/api/vods/${sel.id}`, { title:sel.title, folder:sel.folder, url:sel.url, ts:updatedTs })
      .then(()=>{
        const updated = { ...sel, ts:updatedTs };
        setVods(p=>p.map(v=>v.id===sel.id?updated:v));
        setSel(updated);
      })
      .catch(()=>{
        const updated = { ...sel, ts:updatedTs };
    ...
    setShowTs(false);
    setTsForm({ time:"0:00", cat:"Mistake", note:"" });
  };

  const addReply = (ti, text) => {
    if(!text.trim()||!sel) return;
    const updatedTs = sel.ts.map((t,i)=>i===ti?{...t,replies:[...(t.replies||[]),text]}:t);
    api.put(`/api/vods/${sel.id}`, { title:sel.title, folder:sel.folder, url:sel.url, ts:updatedTs })
      .then(()=>{
        const updated = { ...sel, ts:updatedTs };
        setVods(p=>p.map(v=>v.id===sel.id?updated:v));
  ...
  const deleteVod = (id) => {
    api.delete(`/api/vods/${id}`).catch(()=>{});
    setVods(p=>p.filter(v=>v.id!==id));
    ...
    <div style={{ padding:"28px 32px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"200px 1fr", gap:18 }}>
        <div>
          ...
          {loading
            ? <div style={{ color:"var(--t3)", fontSize:12, padding:"4px 0" }}>Loading...</div>
            ...
            : filteredVods.map(v=>(
              <div key={v.id} style={{ position:"relative", marginBottom:5 }}>
                <div onClick={()=>setSel(v)} style={{ padding:"10px 11px", paddingRight:28, borderRadius:"var(--r2)", cursor:"pointer",
                ...
                <button onClick={()=>deleteVod(v.id)} style={{ position:"absolute", top:6, right:6, background:"transparent", border:"none", color:"var(--t3)", cursor:"pointer", fontSize:12, padding:"2px 4px", borderRadius:3 }} title="Delete">✕</button>
              </div>
            ))
          }
        </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            <div className="card" style={{ padding:"14px 18px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                ...
                <button className="btn btn-sub" style={{ fontSize:11 }} onClick={()=>setShowTs(true)}>+ Timestamp</button>
              </div>
            </div>
            <div className="card">
      ...
      {showTs && sel && (
        <Modal onClose={()=>setShowTs(false)} title="Add Timestamp">
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div><div className="label-sm" style={{ marginBottom:6 }}>Timestamp</div><input type="text" value={tsForm.time} onChange={e=>setTsForm(f=>({...f,time:e.target.value}))} placeholder="e.g. 1:42"/></div>
            ...
            <div><div className="label-sm" style={{ marginBottom:6 }}>Note</div><textarea rows={3} style={{ resize:"vertical" }} value={tsForm.note} onChange={e=>setTsForm(f=>({...f,note:e.target.value}))} placeholder="What happened at this timestamp?"/></div>
            <div style={{ display:"flex", gap:8 }}>
              <button className="btn btn-acc" style={{ flex:1, justifyContent:"center" }} onClick={addTimestamp} disabled={!tsForm.note.trim()}>Add Annotation</button>
              <button className="btn btn-ghost" onClick={()=>setShowTs(false)}>Cancel</button>
            </div>
          </div>
        </Modal>
      )}
